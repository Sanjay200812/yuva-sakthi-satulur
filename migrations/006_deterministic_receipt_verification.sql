-- Migration 006: Deterministic Receipt Verification Schema & Admin Payment Settings
...
BEGIN
  ...
  -- 2. Add columns to payment_submissions for deterministic verification
  ...
  -- 3. Create partial unique index on confirmed transaction reference hashes to prevent double redemption
  CREATE UNIQUE INDEX IF NOT EXISTS idx_payment_submissions_ref_hash_confirmed
    ON payment_submissions(payer_utr_hash)
    WHERE status = 'payment_confirmed';

  CREATE UNIQUE INDEX IF NOT EXISTS idx_payment_submissions_entered_ref_confirmed
    ON payment_submissions(entered_reference_hash)
    WHERE status = 'payment_confirmed';
  ...
  -- 4. Update check constraints on payment_submissions
