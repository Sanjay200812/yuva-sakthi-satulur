...
export async function generateTopRegionCandidate(rawBuffer: Buffer): Promise<Buffer> {
  const sharp = await getSharp();
  if (!sharp) return rawBuffer;
  try {
    const metadata = await sharp(rawBuffer).metadata();
...
export function normalizeOcrText(text: string): string {
  ...
  return text
    ...
    .replace(/@yb[|1Il]\b/gi, '@ybl') // Repair OCR typo on @ybl handle
...
export function extractAmount(
  ...
  // Also scan all lines for standalone amounts (e.g. standalone "50" on FamPay/GPay/PhonePe header crops)
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const sm of standaloneMatches) {
      ...
      if (!isNaN(num) && num >= 10 && num <= 50000) {
        let score = 30;
        if (expectedAmount !== undefined && Math.abs(num - expectedAmount) < 0.05) {
          score = 90;
        }
    ...
    // Check for OCR rupee symbol confusion prefix: e.g. 350 -> ₹50, I50 -> ₹50, ?50 -> ₹50
...
export function extractPaymentStatus(text: string): { status: 'success' | 'pending' | 'failed' | 'unknown'; confidence: number } {
  ...
  if (
    ...
    /\b(?:paid to|paid with|paid via|paid|completed|successful|sent)\b/.test(lower)
  ) {
...
export function detectAiGeneratorWatermark(text: string): boolean {
  ...
  // Explicit watermark strings only
  if (
  ...
  if (mockOcrText !== null) {
    ...
    try {
      ...
      const ocrPromise = (async () => {
        ...
        if (!hasRef || !hasAmount) {
          try {
            const secondaryCandidate = !hasAmount
              ? await generateTopRegionCandidate(sanitizedBuffer)
              : await generateSecondaryContrastCandidate(sanitizedBuffer);
